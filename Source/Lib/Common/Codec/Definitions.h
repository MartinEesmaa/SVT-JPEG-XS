...
#define JPEGXS_SVT_CREATE_SEMAPHORE(pointer, initial_count, max_count)   \
    do {                                                          \
        pointer = jpegxs_svt_create_semaphore(initial_count, max_count); \
        SVT_ADD_MEM(pointer, 1, POINTER_TYPE_SEMAPHORE);          \
    } while (0)

#define JPEGXS_SVT_DESTROY_SEMAPHORE(pointer)                             \
    do {                                                           \
        if (pointer) {                                             \
            jpegxs_svt_destroy_semaphore(pointer);                        \
...
#define SVT_CREATE_MUTEX(pointer)                    \
    do {                                             \
        pointer = jpegxs_svt_create_mutex();                \
        SVT_ADD_MEM(pointer, 1, POINTER_TYPE_MUTEX); \
    } while (0)

#define JPEGXS_SVT_DESTROY_MUTEX(pointer)                             \
    do {                                                       \
        if (pointer) {                                         \
            jpegxs_svt_destroy_mutex(pointer);                        \
