    ...
    if (frame_pool) {
        ...
        if (image_config == NULL) {
            ...
            if (frame_pool->pool_image_buffer_resource_ptr) {
                frame_pool->pool_image_buffer_fifo_ptr = jpegxs_svt_system_resource_get_producer_fifo(
        ...
        if (frame_pool) {
            if (bitstream_size == 0) {
                ...
                if (frame_pool->pool_bitstream_resource_ptr) {
                    frame_pool->pool_bitstream_fifo_ptr = jpegxs_svt_system_resource_get_producer_fifo(
    ...
    if (frame_pool && frame) {
        ...
        if (blocking_flag) {
            if (frame_pool->use_image_buffer) {
                jpegxs_svt_get_empty_object(frame_pool->pool_image_buffer_fifo_ptr, &wrapper_ptr_image_ctx);
            }
            if (frame_pool->use_bitstream) {
                jpegxs_svt_get_empty_object(frame_pool->pool_bitstream_fifo_ptr, &wrapper_ptr_bitstream_ctx);
            }
        }
        else {
            if (frame_pool->use_image_buffer) {
                jpegxs_svt_get_empty_object_non_blocking(frame_pool->pool_image_buffer_fifo_ptr, &wrapper_ptr_image_ctx);
                if (wrapper_ptr_image_ctx == NULL) {
                    return SvtJxsErrorNoErrorEmptyQueue;
                }
            }
            if (frame_pool->use_bitstream) {
                jpegxs_svt_get_empty_object_non_blocking(frame_pool->pool_bitstream_fifo_ptr, &wrapper_ptr_bitstream_ctx);
                if (wrapper_ptr_bitstream_ctx == NULL) {
                    if (wrapper_ptr_image_ctx) {
                        jpegxs_svt_release_object(wrapper_ptr_image_ctx);
                    }
                    return SvtJxsErrorNoErrorEmptyQueue;
                }
            }
...
PREFIX_API void svt_jpeg_xs_frame_pool_release(svt_jpeg_xs_frame_pool_t* frame_pool, svt_jpeg_xs_frame_t* frame) {
    if (frame_pool && frame) {
        if (frame_pool->use_image_buffer && frame->image.release_ctx_ptr && frame->image.ready_to_release) {
            jpegxs_svt_release_object(frame->image.release_ctx_ptr);
        ...
        if (frame_pool->use_bitstream && frame->bitstream.release_ctx_ptr && frame->bitstream.ready_to_release) {
            jpegxs_svt_release_object(frame->bitstream.release_ctx_ptr);
